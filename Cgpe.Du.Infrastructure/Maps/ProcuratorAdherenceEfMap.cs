...
namespace Cgpe.Du.Infrastructure
{
    ...
    public class ProcuratorAdherenceEfMap
    {
        ...
        public void Map(ProcuratorAdherence source, ProcuratorAdherenceEntity target, Guid procuratorId, Guid? addressId, Guid? phoneId, Guid? mobileId, Guid? emailId, bool isNew = false)
        {
            if (isNew)
            {
                source.ProcuratorAdherenceId = Guid.NewGuid();
            }
