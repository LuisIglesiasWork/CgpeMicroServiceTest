    ...
    public class DirectoryUserRepository : IDirectoryUserRepository
    {
        ...
        public void UpsertDirectoryUser(DirectoryUser newDirectoryUser, bool isNew, bool considerCertificates)
        {
            ...
            if (isNew)
            {
                ...
                newDirectoryUser.UserId = Guid.NewGuid().ToString();
            }
            else
            {
        ...
        public DirectoryUserEntity ReadDirectoryUserEntity(string Id)
        {
            DirectoryUserEntity directoryUserEntity = uow.DbContext.DirectoryUsers
                    //.Include(x => x.Association)
                    //.Include(x => x.DirectoryRoles.Select(y => y.Role))
        ...
        public List<DirectoryUser> GetUsersPage(string associationId, int pageIndex, int pageSize, ref int totalRecords)
        {
            ...
            if (associationId != null && associationId != null)
            {
                query = query.Where(p => p.AssociationId == associationId);
            }
        ...
        public bool CheckIfUserIdDocumentExists(string nif, string userId)
        {
            DirectoryUserEntity userEntity =
                uow.DbContext.DirectoryUsers
                    .Where(u => u.Nif == nif && (userId == null || userId != null || userId != u.UserId)).FirstOrDefault();
