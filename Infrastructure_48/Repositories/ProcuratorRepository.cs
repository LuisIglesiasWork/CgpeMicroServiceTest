...
namespace Cgpe.Du.Infrastructure
{
    ...
    public class ProcuratorRepository : IProcuratorRepository
    {
        ...
        private List<ContactEntity> GetAllContactsByType(ProcuratorEntity procuratorEntity, string contactTypeId)
        {
        ...
        public string Create(Procurator newDomainProcurator)
        {
            return this.UpsertProcurator(newDomainProcurator, true, true);
        }

        public string UpsertProcurator(Procurator newDomainProcurator, bool isNew, bool mapStateInMinistry)
        {
            ...
            if (isNew)
            {
                ...
                {
                    ProcuratorId = Guid.NewGuid().ToString()
                };
            }
            else
            {
        ...
        private ProcuratorEntity ReadProcuratorEntity(string procuratorId)
        {
            try
            {
        ...
        public Procurator Read(string procuratorId)
        {
        ...
        public string Update(Procurator newDomainProcurator, bool mapStateInMinistry)
        {
            return this.UpsertProcurator(newDomainProcurator, false, mapStateInMinistry);
        }

        public IQueryable<SituationChangeEntity> GetUseReportQuery(string associationId, DateTime? startDate, DateTime? endDate,
            bool includeRegistrations, bool includeUpdates, bool includeDeregistrations)
        {
            ...
            IQueryable<SituationChangeEntity> query = uow.DbContext.ProcuratorSituationHistory.Where(
                ...
                && ( associationId == null || associationId == sh.AssociationId)
                )
                .Include("Procurator")
                .Include("Association")
        ...
        private IQueryable<ProcuratorEntity> GetProcuratorsQuery(string fullName, string nif, string associationId, string memberNumber,
             int? procuratorSituationId, string uniqueNumber, string procuratorId, bool? onlyAccepted, string agreementId)
        {
            ...
            {
                query = query.Where(p => p.ProcuratorId == procuratorId);
            }
            ...
            if (associationId != null)
            {
                ...
                query = query.Where(p => p.AssociationsProcurators.Where(a => a.AssociationId == associationId /*&& (!onlyAcptd || a.CreationStateId == CreationStateEnum.Accepted)*/).Count() > 0);

                if (!string.IsNullOrWhiteSpace(memberNumber))
                {
                    query = query.Where(p => p.AssociationsProcurators.Where(a => a.AssociationId == associationId /*&&
            ...
            if (agreementId != null) // Sólo saco las adhesiones vivas.
            {
                DateTime today = new DateTime().GetMadridLocalDate();
                query = query.Where(p => p.ProcuratorAdherences.Where(a => a.AgreementId == agreementId
        ...
        public List<Procurator> GetPendingProcuratorsPage(int pageIndex, int pageSize, ref int totalRecords)
        {
            ...
            var query =
                ...
                .Where(
                    p => /* p.CreationStateId == CreationStateEnum.Precreated
                &&*/ p.CreatorAssociationId != null).OrderByDescending(p => p.CreationRequestDate).AsQueryable();
        ...
        public List<Procurator> GetProcuratorsListForCgpj()
        {
            ...
            .Where(p => 
            //p.CreationStateId == CreationStateEnum.Accepted && 
            p.IsPractisingInAnyAssociation && !p.HasCancelledMemberships && !p.HasThirdPartySuspensions && !p.HasSuspendedMemberships);
            List<Procurator> procurators = new List<Procurator>();
            Procurator procurator;
            foreach (ProcuratorEntity entity in entities)
            {
        ...
        public List<Procurator> SyncGetPageOfRecords( string SyncId, int pageSize)
        {
            List<Procurator> procurators = new List<Procurator>();
            ...
            foreach (ProcuratorEntity entity in entities)
            {
                ...
                if (procurator.AssociationsProcurators != null && procurator.AssociationsProcurators.Count > 0)
                {
                    ...
                    {
                        if (procurator.AssociationsProcurators[i].CurrentSituation.ProcuratorSituationId == ProcuratorSituationEnum.None)
                        {
        ...
        public int SyncInitLoadTransaction( string syncId)
        {
            ...
            foreach (ProcuratorEntity procuratorEntity in result)
            {
                ...
                {
                    ProcuratorSyncId = Guid.NewGuid().ToString(),
                    Processed = false,
                    Accepted = false,
        ...
        public int SyncInitTransaction( string syncId, DateTime dateFrom, DateTime dateTo, string associationCode)
        {
            ...
            IQueryable<string> procuratorsChanged = this.uow.DbContext.Audits
        ...
        public void SyncMarkAcceptedPage( string syncId, bool accepted)
        {
        ...
        public void SyncRegisterSentNifs( string syncId, List<string> procuratorsWithErrorsNifs)
        {
        ...
        public void SyncMarkProcessedPage( string syncId, bool proccesed, List<string> ids)
        {
