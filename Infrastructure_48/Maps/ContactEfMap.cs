    ...
    public class ContactEfMap
    {
        ...
        public void Map(ContactEntity source, Contact target)
        {
            ...
            if (source.AssociationId != null)
                target.Association = new Association() { AssociationId = source.AssociationId };
            if (source.ProcuratorId != null)
                target.Procurator = new Procurator() { ProcuratorId = source.ProcuratorId };
            if (source.AddressId != null)
                target.Address = new Address() { AddressId = source.AddressId };
        }

        public void Map(Contact source, ContactEntity target, string procuratorId, string addressId, string associationId, bool isNew = false)
        {
            if (isNew)
            {
                source.ContactId = Guid.NewGuid().ToString();
            }
            target.ContactId = source.ContactId;
